...
init(#{uid := UId} = Conf) ->
    ...
    Reader = ra_log_reader:init(UId, MaxOpen, SegRefs),
...
handle_event({segments, Tid, NewSegs},
    ...
    Effs = ra_log_reader:emit(Readers, Reader),
    {State, Effs};
...
delete_segments(Idx, #?MODULE{log_id = LogId,
    ...
    case ra_log_reader:update_first_index(Idx, Reader0) of
        {Reader, []} ->
            {State0#?MODULE{reader = Reader},
             ra_log_reader:emit(Readers, Reader)};
