...
external_reader(Config) ->
    ...
    Pid = spawn(
            fun () ->
                    receive
                        {ra_log_update, _UId, FirstIdx, Segments} = Evt ->
                            R1 = ra_log_reader:handle_log_update(FirstIdx,
                                                                 Segments,
                                                                 R0),
                            {Es, _, R2} = ra_log_reader:read(FirstIdx, 220, R1),
                            Len1 = length(Es),
                            ct:pal("Es ~w", [Len1]),
                            Self ! {got, Evt, Es},
                            receive
                                {ra_log_update, _, F, S} = Evt2 ->
                                    %% reader before update has been processed
                                    %% should work
                                    {Stale, _, _} = ra_log_reader:read(F, 220, R2),
                                    ?assertEqual(Len1, length(Stale)),
                                    ct:pal("S2 ~w ~w",
                                           [S, ets:tab2list(ra_log_open_mem_tables)]),
                                    R3 = ra_log_reader:handle_log_update(F, S, R2),
    ...
    {Log2, [_, {send_msg, Pid, Update, _}]} = ra_log:register_reader(Pid, Log1),
    Pid ! Update,
    ...
    deliver_all_log_events(Log2, 500),
