...
start_stop_restart_delete_on_remote(Config) ->
    ...
    % idempotency
    ok = ra:stop_server(NodeId),
    ok = ra:force_delete_server(default, NodeId),
    % idempotency
    ok = ra:force_delete_server(default, NodeId),
    timer:sleep(500),
    slave:stop(S1),
...
start_cluster(Config) ->
    ...
    Machine = {module, ?MODULE, #{}},
    {ok, Started, []} = ra:start_cluster(default, ClusterName, Machine, NodeIds),
...
start_or_restart_cluster(Config) ->
    ...
    Machine = {module, ?MODULE, #{}},
    %% this should start
    {ok, Started, []} = ra:start_or_restart_cluster(default, ClusterName, Machine,
                                                    NodeIds),
    % assert all were said to be started
    [] = Started -- NodeIds,
    % assert all nodes are actually started
    ...
    NodeIds = [{ClusterName, start_follower(N, PrivDir)} || N <- [s1,s2,s3]],
    %% this should restart
    {ok, Started2, []} = ra:start_or_restart_cluster(default, ClusterName, Machine,
                                                     NodeIds),
    [] = Started2 -- NodeIds,
    timer:sleep(1000),
...
delete_one_server_cluster(Config) ->
    ...
    Machine = {module, ?MODULE, #{}},
    {ok, _, []} = ra:start_cluster(default, ClusterName, Machine, NodeIds),
    [{_, Node}] = NodeIds,
    ...
    undefined = rpc:call(Node, ra_directory, uid_of, [default, ClusterName]),
...
start_cluster_majority(Config) ->
    ...
    {ok, Started, NotStarted} =
        ra:start_cluster(default, ClusterName, Machine, NodeIds),
